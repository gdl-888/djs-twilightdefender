...
var helpMsg = 	"twilight-defender 버전 " + botVer + " - 말을 걸거나 관리 기능을 사용합니다\n" +
				"\n" +
...
client.on('message', (msg) => { /* 봇에 메시지가 올 때 */
		...
		if(warningList[msg.member.toString()] >= 3) {
			...
			try {
				if(!msg.member.toString().includes("672008827276623874")) {
					...
					msg.member.removeRole('670566324660994078'); /* 정회원 */
					msg.member.removeRole('670427230429642792');
	...
	/* 사용자 차단 */
		...
		if(msg.member.roles.find(r => r.name === "관리자")){
			try {
				...
				memberObj.removeRole('670427230429642792');
	...
	/* 권한 부여 */
		...
		if(msg.member.roles.find(r => r.name === "정회원")){
			try {
				var memberObj = msg.guild.members.get(userID) /* client.users.find(user => user.id == userID) */;
				var username = client.users.find(user => user.id == userID)['username'];
				...
				switch(grType) {
					case 'B':
						...
						msg.channel.send('[["' + String(username) + '" 권한 부여 - "+준회원 "]]');
						break;
					case 'A':
						if(msg.member.roles.find(r => r.name === "관리자")) {
	...
	/* 사용자 차단해제 */
		...
		if(msg.member.roles.find(r => r.name === "관리자")){
			try {
				...
				memberObj.addRole('670427230429642792');
