...
var helpMsg = 	"버전 " + botVer + " - 말을 걸거나 관리 기능을 사용합니다\n" +
				"\n" +
				"`    ![문장|[도움말|/?]]`\n" +
				"\n" +
				"`        문장 - 봇에게 말할 문장입니다.`\n" +
				"`        도움말 - 도움말 내용을 표시합니다.`\n" +
				"\n\n" +
				"`    /[[/B] [ID]] [[/U] ID] [[/G] [+A|+B] ID] [[/D] ID] [[/P] ID] [[/W] ID] [[/R] ID] [/C] [/V] [/K [가위|바위|보]] [/O]`\n" +
				"\n" +
				"`        /B - 지정한 사용자를 차단합니다`\n" +
...
<봇 치트> \n\
    `$MKSWEAR`: 봇을 욕하게 만든다\n\
    `$SWEARWRD_CHT`: 봇이 욕설로 인식하여 이 메시지를 보낸사람에게 경고를 준다\n\
	`$MK_BNYSF`: 봇이 자신을 차단하는것을 시도하게 한다";

var warningList = {};

client.on('message', (msg) => { /* 봇에 메시지가 올 때 */
	if(msg.channel.id == '671991154115608586') {
		switch(msg.content) {
			case '1':
				msg.member.addRole('670566324660994078');
			break ; case '2':
				msg.member.addRole('670427230429642792');
			break ; case '3':
				client.channels.get('671991154115608586').send(
					'If you know <@453432847617884172>, please wait until he is online... \n' + 
					'<@453432847617884172>이 누구인지 안다면 해당 사용자가 온라인일 때까지 대기해주십시오...'
				);
		}
	}
	...
	/*
	if(msg.channel.id == '671991154115608586') {
		if(!(msg.content.includes("권한")) && !(msg.content.includes("요청"))) {
			msg.delete(1000);
		}
	}
	*/
	...
	if(imsg === "$MK_BNYSF") {
		msg.channel.send("//B 672008827276623874");
	}
	
	if(imsg.match(/^twilight[-]defender\s{0,10}[/]/i)) {
		msg.channel.send("기능 스위치를 사용하려면 /[스위치]를 사용하십시오.");
	}
	...
	if( 
		...
		|| imsg.includes('混蛋 ') || imsg.includes('我靠') || imsg.includes('我尻') || imsg.includes('牛屄') || imsg.includes('拍马屁')
		 || imsg.includes('滚开') || imsg.includes('滚开') || imsg.includes('靠北') || imsg.includes('混賬') || imsg.includes('你丫挺的')
		  || imsg.includes('妈的') || ismsg.includes('나가뒤져') || imsg.includes('ㅆㄲ') || imsg.includes('양아치')
	) {
		var SML = [
			"아니.. 고운 말을 사용합시다!",
			"욕을 쓰면 안 돼요...",
		...
		if(warningList[msg.member.toString()] >= 3) {
			...
			var banDuration = 10 * ((10 + msg.content.length) / 10)
	
			msg.channel.send('욕설이 세 번 이상 감지됐읍니다. ' + String(banDuration) + '분 후에 자동 해제됩니다. 이 차단이 오류가 있다고 생각하시면 차소게에 설명을 해주십시오.');
			...
			try {
				if(!msg.member.toString().includes("672008827276623874")) {
					...
					client.channels.get('684395442397642774').send(getDateTime() + " **twilight-defender** 사용자가 **" + username + "** *(사용자 차단)* (" + String(banDuration) + "분 동안) (욕설 사용)");
				} else {
	...
	*/
	...
	else if(msg.content.toUpperCase().startsWith("!")) {
		print("<<< " + msg.content);
		...
		else if(message.includes('안녕히') || message.includes('ㅂㅇ') || message.includes('바이') || message.includes('뽜이') || message.includes('빠이')) {
			sendMsg = '안녕히 가십시오.';
			msg.channel.send(sendMsg);
			print(">>> " + sendMsg);
		}
		else if(message.includes('안녕') || message.includes('ㅎㅇ') || message.includes('하이')) {
			sendMsg = '안녕하십니까.';
			msg.channel.send(sendMsg);
			print(">>> " + sendMsg);
		}
		...
		else if(message.includes('가위바위보')) msg.channel.send("사용법: `//K [가위|바위|보]`");
		else if(message.includes('업다운') || message.includes('UPDOWN')) msg.channel.send("사용법: `//O");
		else if(
	...
	else if(msg.content.match(/^[/][/][B]\s{0,10}$/i) || msg.content.match(/^[/][/][B]\s{1,10}\D{1,50}$/i)) {
		var userID = msg.content.replace(/^[/][/][B]\s{0,10}/i, '');
		msg.channel.send("매개 변수가 틀립니다 - " + userID);
	}
	
	/* 사용자 차단 */
	else if(msg.content.match(/^[/][/][B]\s{1,10}\d{1,50}$/i)) {
		print("<<< 차단요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][B]\s{1,10}/i, '');
	...
	/* 경고 부여 */
	else if(msg.content.match(/^[/][/][W]\s{1,10}\d{1,50}$/i)) {
		print("<<< 경고요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][W]\s{1,10}/i, '');
		try {
	...
	/* 경고 해제 */
	else if(msg.content.match(/^[/][/][R]\s{1,10}\d{1,50}$/i)) {
		print("<<< 경고요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][R]\s{1,10}/i, '');
		try {
			if(msg.member.roles.find(r => r.name === "관리자") || msg.member.roles.find(r => r.name === "중재자")){
				var username = client.users.find(user => user.id == userID)['username'];
				var userIDs = "<@" + String(userID) + ">";
	...
	/* 준회원 강등 */
	else if(msg.content.match(/^[/][/][D]\s{1,10}\d{1,50}$/i)) {
		print("<<< 강등요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][D]\s{1,10}/i, '');
		if(msg.member.roles.find(r => r.name === "관리자")){
			try {
				var memberObj = msg.guild.members.get(userID) /* client.users.find(user => user.id == userID) */;
				var username = client.users.find(user => user.id == userID)['username'];
	...
	/* 정회원 승급 */
	else if(msg.content.match(/^[/][/][P]\s{1,10}\d{1,50}$/i)) {
		print("<<< 승급요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][P]\s{1,10}/i, '');
		if(msg.member.roles.find(r => r.name === "관리자") || msg.member.roles.find(r => r.name === "중재자")){
			try {
				var memberObj = msg.guild.members.get(userID) /* client.users.find(user => user.id == userID) */;
				var username = client.users.find(user => user.id == userID)['username'];
	...
	/* 가위바위보 */
	else if(msg.content.match(/^[/][/][K]/i)) {
		if(msg.content.match(/^[/][/][K]\s{1,10}(가위|바위|보)/i)) {
			var fng = [
				"가위",
				"바위",
				"보"
			...
			msg.channel.send("사용법: `//K [가위|바위|보]`");
		}
	}
	
	/* 업다운 */
	else if(msg.content.match(/^[/][/][O]/i)) {
		if(isup == true && updu != msg.member.toString()) {
			msg.channel.send("지금은 다른 사용자가 하고있읍니다.");
		} else {
			try {
				msg.channel.send("업다운 게임\n제한시간은 30초입니다 - 포기하려면 `//N / 숫자를 말하려면 그냥 대화창에 숫자를 쓰시오.`");
	...
	/* 권한 부여 */
	else if(msg.content.match(/^[/][/][G]\s{1,10}[+](A|B)\s{1,10}\d{1,50}$/i)) {
		print("<<< 권한 부여요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][G]\s{1,10}[+](A|B)\s{1,10}/i, '');
		var grType = msg.content.replace(/^[/][/][G]\s{1,10}[+]/i, '').replace(/\s{1,10}\d{1,50}$/i, '').toUpperCase();
	...
	/* 사용자 차단해제 */
	else if(msg.content.match(/^[/][/][U]\s{1,10}\d{1,50}$/i)) {
		print("<<< 차단 해제요청이 들어왔읍니다.");
		var userID = msg.content.replace(/^[/][/][U]\s{1,10}/i, '');
	...
	else if(msg.content.replace(/\s/g, '') === "twilight-defender/?" || msg.content.replace(/\s/g, '') === "//?") {
		print("<<< 사용자가 사용법을 요청했읍니다");
		msg.channel.send(helpMsg);
	}
	else if(msg.content.replace(/\s/g, '').toUpperCase() === "//Z") {
		print("<<< 사용자가 치트사용법을 요청했읍니다");
		msg.channel.send(chtHlp);
	}
	else if(msg.content.replace(/\s/g, '').toUpperCase() === "//V") {
		msg.channel.send("twilight-defender 버전 " + botVer + "\n" + 
				"개정 " + rev + "\n" +
				"경고 초기화까지 " + String(12 - time) + "시간 남았읍니다");
	}
	else if(msg.isMemberMentioned(client.user) || (message.includes('twilight-defender') && msg.content !== "twilight-defender /?" && !(msg.member.toString().includes('672008827276623874')))) {
		print("<<< " + msg.content);
		sendMsg = '4.0 이후 호출법이 변경됐습니다. 저에게 말을 걸려면\n - 기존: @[봇이름] [내용], 새 방식: ![내용]\n기능을 사용하려면\n - 기존: [봇이름] [매개변수], 새 방식: /[매개변수]\n - 기능 도움말을 보려면 `//?`를 입력하십시오.';
		msg.channel.send(sendMsg);
		print(">>> " + sendMsg);
	}
});
...
Enter 1 if you are here to discuss about coding
Enter 2 if you are here to just chat or play games
Enter 3 if you actually know <@453432847617884172> (Will be verified manually)

프로그래밍에 대해 토론하러 들어왔다면 1을,
단순 대화하거나 게임하기 위해 들어왔다면 2를,
<@453432847617884172>가 누구인지 알거나 만난 적이 있다면 3(이 경우는 수동 인증될 예정)을 입력하십시오.
`
;
